...
import RtfParser
import sys
#import tkinter as tk # Uncomment for Python 3.5
import Tkinter as tk #//-> Uncomment for Python 2.7
import tkFileDialog #//-> Uncomment for Python 2.7
import os
import ctypes
...
if __name__ == '__main__':
    #Closes the Tkinter pop windows
    tk.Tk().withdraw()

    # Opens a dialog window to pick multiple files to translate
    #file_name = tk.filedialog.askopenfilename() #//-> Uncomment for Python 3.5
    try:

        home = os.path.expanduser('~')
        file_name = tkFileDialog.askopenfilenames(initialdir=home, filetypes = [ 
            ("RTF Files", ("*.rtf")) ] ) #//-> Uncomment for Python 2.7
        
        file_list = list(file_name)
        
        for file in file_list:
            finput = open(file,'r')
            foutput = open(str(finput.name).lower().replace('.rtf','.txt'),'w')
            tester = Rtf2Txt(foutput)
            for line in finput:
                tester.feed(line)
            tester.close()
            foutput.close()
        ...
        if len(file_list) > 0:
            message = "%s files were converted." % (len(file_list))
            ctypes.windll.user32.MessageBoxA(0, message, "Rtf2Txt",0)
